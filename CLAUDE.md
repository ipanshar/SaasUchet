...
После завершения любой задачи или фичи:

1. Сделать `git commit` с кратким описанием на **русском языке** (не более 200 символов).
2. Сразу запушить в ветку **`main`** (`git push origin main`).
3. Не ждать отдельной команды — пушить автоматически после каждого завершённого изменения.

## Тесты

**Тесты писать не нужно.** Разработчик проводит тестирование вручную самостоятельно. Не создавать тестовые файлы, не добавлять тест-кейсы, не запускать `go test` или `flutter test` в рамках разработки.
