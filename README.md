...
Временный seed-скрипт наполняет стенд демо-компанией (мебельный бизнес): 7 пользователей с ролями (пароль у всех `12345678`), связанные сотрудники на окладах, контрагенты-должники и кредиторы, каталог/услуги, производство, ~200 документов продаж/закупок за 3 месяца и проведённые зарплаты.

При поднятых PostgreSQL и бекенде (`localhost:8080`):

```bash
cd backend && go run ./cmd/seeddemo
```

Каждый прогон создаёт независимую компанию с уникальным суффиксом — запускать можно многократно. По окончании скрипт печатает телефоны для входа. URL API переопределяется переменной `API_BASE_URL`.
